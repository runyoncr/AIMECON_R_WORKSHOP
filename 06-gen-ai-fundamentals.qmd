...
This differs from a traditional regression workflow, where you often specify the predictor variables based on domain knowledge. Regression can also be used as a machine learning method when the model is trained to predict outcomes from data.
With many other machine learning approaches, you instead supply a large set of potentially relevant variables and allow the algorithm to identify patterns that improve prediction.
Here is where we may start to encounter so-called “black box” models, because interpreting the decision-making processes of complex algorithms can be difficult.
...
LLMs are deep learning models trained on large amounts of language data to model and generate human language.
...
Some language models are designed primarily for tasks such as classification, embedding, or reranking rather than open-ended generation.
...
This kind of extrapolation beyond the training data can contribute to hallucinations, or factual and structural errors, but it is not the only source of such errors.
...
Training mixtures vary by model and may include public, licensed, and human-generated data; providers do not always disclose their complete datasets.
Large and varied training data can give a model broad statistical knowledge of language, but it does not guarantee factual accuracy or complete coverage of any topic.
This understanding is really a recognition of patterns of word usage across many contexts.

It is important to note that these models do not inherently verify the factual accuracy of the information they provide.
...
Autoregressive text models generally generate one token at a time based on patterns learned from prior text. External tools, retrieval, or a separate verification step can improve factual reliability, but they do not make errors impossible.

### Transformer

And, finally, **T**—transformer. 
This refers to the specific neural network architecture that makes models like GPT so powerful. 
Before transformers were introduced, recurrent models processed language sequentially, which limited their ability to capture long-range context and parallelize computations efficiently.
The transformer architecture changed this by allowing the model to compare positions across a sequence through attention and to process training sequences in parallel. Autoregressive transformer models still generate output tokens sequentially.
...
[This visual explainer from the Financial Times](https://ig.ft.com/generative-ai/) introduces transformers and related concepts without being too technical.