...
Although traditional validity frameworks were developed before LLMs were ubiquitous and widely adopted, the same basic principles from these frameworks still apply.
There are at least two perspectives on integrating LLM-supported workflows into validity frameworks:
...
![](images/Workflow_Cropped.png){fig-align="center" width=50% fig-alt="A high-level workflow diagram showing iterative prompt development followed by LLM workflow outcome evaluation."}
...
![](images/Prompt_Revision_Cropped.png){fig-align="center" fig-alt="A workflow diagram focused on revising and testing prompts before using them in a broader LLM workflow."}
...
And, equally importantly, any important detail that is omitted is something the model will generate on its own, which may be problematic for specific tasks[^1].
One strategy for catching these gaps before a prompt enters a workflow is **metaprompting** (sometimes written **meta-prompting**): asking an LLM to critique, clarify, or improve the prompt itself before you use it for the downstream task.

The simple idea at the start of the workflow is: make a prompt &rarr; test it &rarr; see if the output of the prompt is good.
For example, if you're working on a content generation prompt, you'll want to work carefully on the prompt, apply it to a handful of different use cases (content areas, etc), and then look _across the different outputs_ to see what worked and what didn't.
...
When you get a suboptimal output, it also takes time to think about how to prevent that from happening again by reverse-engineering additional prompt instructions or adjustments. 
...
We'll practice this prompt engineering process [in an activity later](16-activity-iterative-prompt-development.qmd).

**It should be expected that you will have to make some adjustments to your prompt.** 
This is not a shortcoming of your prompt engineering skills or the LLM; sometimes you just don't realize what some of the nuances of the prompt-output relationship will produce.
...
For example, if you're using a prompt to generate content for a specific subject, having the same SMEs that you would trust with item writing involved in the initial prompt crafting can be incredibly helpful.
...
![](images/Single_Prompt_Cropped.png){fig-align="center" fig-alt="A workflow diagram showing candidate prompt versions tested across the same candidate use cases before selecting and revising a prompt."}
...
LLMs are the same way - anything that is important to complete the task that you don't specify, the model will have to generate on its own.
This is especially important when there is a sequential or logical dependence among the outputs that you're developing, and it gives you more flexibility to alter the workflow at different stages.

When using a sequential prompt workflow, the same evaluation for a single prompt generalizes to the multi-step prompts.
The only wrinkle is that sometimes when evaluating later prompts (e.g., 1c below) you can catch that you might need to change something in your initial prompt (1a) to improve that later output because of the sequential nature of the build.
Visually, it might look something like this:

![](images/Complex_Prompt_Cropped.png){fig-align="center" fig-alt="A workflow diagram showing a complex prompt broken into multiple sequential prompt steps that can be tested and revised."}
...
![](images/LLM_Eval_Cropped.png){fig-align="center" fig-alt="A workflow diagram showing outcome evaluation after initial prompt refinement."}

This part of the workflow likely mirrors existing non-LLM-supported processes in your workflow.
...
Fitness for that purpose requires a different evaluative lens, bringing us full circle to the validity point at the beginning of this section.
