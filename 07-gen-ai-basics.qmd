...
_For example, the sentence “Educational measurement is evolving quickly.” might be split into tokens like [Educational] [ measurement] [ is] [ evolving] [ quickly] [.]._
...
Models typically use a method called Byte Pair Encoding (BPE) or one of its descendants [@sennrich2015neural] [@radford2019language], which balances efficiency (smaller vocabularies) and linguistic flexibility (ability to represent rare or compound words).
...
This is where “token count” matters in API costs: it determines both model cost and how much text fits within a prompt.
Understanding tokenization helps demystify why AI models occasionally truncate responses or miscount words—they’re counting in tokens, not characters or words.
...
For those in educational measurement, attention can be understood as a dynamic weighting function, much like how students allocate attention differently to key versus peripheral information when reading. 
In the same way, a transformer’s self-attention layers learn to emphasize informative relationships and de-emphasize noise.
