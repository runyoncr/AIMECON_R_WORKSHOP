...
Run this chunk first. It loads the required packages, sets your API key, sources the canonical workshop helper, and loads the essay data for the activity.
...
source("call_claude_AIMECON26.R")
...
This chapter reuses the workshop's shared `call_claude()` helper instead of redefining a separate local function, so the examples stay consistent with the earlier connection and API-key materials. The helper returns the model's final text output as a single character string, which is exactly what `parse_scores()` expects when you feed the result through `purrr::map_chr()`.
...
The `temperature` parameter controls that randomness. The shared `call_claude()` helper does not set `temperature`, so the loop above ran at the API default (`1.0`), where the model samples probabilistically from its output distribution and repeated calls might produce different scores for the same essay. For this exercise we use Claude Sonnet 4.5 because it supports the `temperature` values being compared. Anthropic's current models documentation marks `temperature` as deprecated for newer models, and even `temperature = 0` does not guarantee identical output. See the [Messages API documentation](https://platform.claude.com/docs/en/api/messages#body-temperature) for the current behavior.

Batch processing makes this kind of multi-rep study cheap and fast. The following code submits 20 calls per essay (200 requests per condition, 400 total) at the default temperature and at `temperature = 0`, so we can see whether and how much scores vary.
...
With default temperature you should see meaningful within-essay score variance (non-zero SDs), especially on the Language criterion where small differences in wording can tip a borderline response either way. At `temperature = 0` the SDs may shrink, but they should not be expected to collapse completely: Anthropic notes that even zero temperature does not guarantee identical output.
