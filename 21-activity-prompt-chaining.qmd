...
<a href="downloads/activity-prompt-chaining.Rmd" download style="display: inline-block; padding: 10px 20px; background-color: #4A90E2; color: black; text-decoration: none; border-radius: 5px;">
  📥 Download `activity-prompt-chaining.Rmd`
</a>


This activity will allow you to practice developing a workflow that involves prompts at several stages.
This workflow can be useful when you need to develop a large item pool with extensive construct coverage.

The example that we'll use for this activity is developing essay prompts for a formative middle school science quiz.
This will include developing the entire ecosystem for the item, from initial development to providing feedback aligned with performance-level descriptors.

This is a simple and artificial example, but will serve to help you get thinking about developing complex workflows for your own purposes. 
All of these materials we'll be working with were all created with an LLM.
In your own setting these materials would likely be created in collaboration with instructional designers, test development, subject-matter experts, etc., but the idea is the same: you can pull in important instructions or guiding materials at each step to improve the model performance.
Also importantly, because these materials are not hard-coded in the prompt, you can easily revise them or swap them out for similar materials, preserving your workflow.

## Step 1: Selecting Guiding Learning Objective
...
# Download the science learning objectives
load(url("https://raw.githubusercontent.com/runyoncr/AIMECON_R_WORKSHOP/main/data/science_los.Rdata"))

# Converting to text to paste into a prompt; could also make it JSON
...
# Helper function to prepare the step 2 output as string text:

clean_json <- function(x) {
  
  # Strip markdown code fencing (```json, ```JSON, ``` alone, etc.)
  x <- gsub("```[a-zA-Z]*\\n?", "", x)
  ...
  # Attempt to extract a JSON object or array if there's surrounding text
  # Matches the outermost { } or [ ] block
...
This activity uses prompt-requested JSON and `clean_json()` so that each intermediate object remains visible and the workflow stays portable across providers. Current Anthropic models also support structured JSON outputs through `output_config.format`, which can guarantee schema-compliant output for supported models. That production-oriented option is not used here because the workshop's canonical `call_claude()` helper accepts text prompts but does not expose a schema argument; the cleanup step is therefore intentional for this exercise.
...
This is where you'd normally include a real writing sample.
For the sake of demonstration, we're going to have Claude generate one for us.
We'll use Claude to generate a synthetic response so that we can continue through the complete workflow without needing a separate student-data file.
...
In the process above I read in .txt files. Claude can also read in PDFs. For other formats - e.g., .docx - you'll have to use another package - such as `officer` to extract the text. Anthropic's current PDF limits are a 32 MB request size and up to 600 pages, or 100 pages when the request context window is under 1M tokens. Check the [PDF support documentation](https://platform.claude.com/docs/en/build-with-claude/pdf-support) before sending larger documents.

The canonical `call_claude()` helper above accepts text prompts only. This bonus therefore demonstrates a separate, direct Messages API request for document content; it is intentionally not a replacement for the shared helper. PDFs are encoded with `base64enc`, while `.docx` content is extracted with the [`officer` package](https://davidgohel.github.io/officer/).
